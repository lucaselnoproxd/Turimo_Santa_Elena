...
export class BeachesSeed implements OnModuleInit {
  ...
  async onModuleInit() {
    const count = await this.beaches.count();
    if (count > 0) return;

    const data: Partial<Beach>[] = [
      {
        slug: 'montanita',
        name: 'Montañita',
      ...
      },
      {
        ...
        gallery: [
          '/images/playas/chuyuipe.jpg',
          '/images/playas/montanita.jpg',
        ],
      },
      {
        slug: 'chipipe',
    ...
    await this.beaches.save(data.map((d) => this.beaches.create(d)));
  }
}
